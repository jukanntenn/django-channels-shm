...
def test_register_full_returns_zero() -> None:
    ...
    from tests.native._types import ShmLayout as Layout
...
def test_lookup_socket_returns_registered_path(layout: ShmLayout) -> None:
    """registry_lookup_socket returns the path for a registered prefix."""
    _ = registry_register(
        layout.region, "abc123", "/tmp/wakeup_abc123.sock", 1, 1, layout.max_processes
    )
    assert (
        registry_lookup_socket(layout.region, "abc123", layout.max_processes)
        == b"/tmp/wakeup_abc123.sock"
    )


def test_lookup_socket_unregistered_returns_none(layout: ShmLayout) -> None:
    """An unregistered prefix has no socket path."""
    assert registry_lookup_socket(layout.region, "nope", layout.max_processes) is None


def test_lookup_socket_dead_slot_returns_none(layout: ShmLayout) -> None:
    """A marked-dead slot no longer resolves to a path."""
    slot = registry_register(
        layout.region, "dead", "/tmp/dead.sock", 1, 1, layout.max_processes
    )
    assert slot != 0
    registry_mark_dead(layout.region, slot)
    assert registry_lookup_socket(layout.region, "dead", layout.max_processes) is None
